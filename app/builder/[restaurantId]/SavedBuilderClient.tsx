...
import { ArrowLeft, Cloud, ExternalLink, Loader2, Palette, QrCode, Save, Settings2, Languages, Soup, Store, UploadCloud, UtensilsCrossed } from "lucide-react";
...
import { getPlanConfig, supportsDailyMenuPhotos, supportsProductPhotos } from "@/lib/plan-config";
  ...
  return (
    <div className="min-h-screen bg-[#fffaf3] text-[#221812]">
      <header className="sticky top-0 z-50 border-b border-[#eadfce] bg-[#fffaf3]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1560px] flex-wrap items-center justify-between gap-3 px-4 py-4 lg:px-8">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Link href={publicPath} target="_blank" className="inline-flex items-center gap-2 rounded-full border border-[#d9cbb8] bg-white px-4 py-2 text-sm font-bold"><ExternalLink size={16} /> Vista pública</Link>
      ...
      <BuilderLayout
        ...
        editor={<div className="space-y-5"><div className="rounded-[1.5rem] border border-[#eadfce] bg-white px-4 py-3 text-sm font-bold text-[#6b594a] shadow-sm md:flex md:items-center md:justify-between"><span>Modo edición limpio: aquí solo cambias datos. Usa “Vista pública” para revisar la carta en otra pestaña.</span><Link href={publicPath} target="_blank" className="mt-3 inline-flex rounded-full bg-[#221812] px-4 py-2 text-xs font-black text-white md:mt-0">Abrir vista pública</Link></div>{planNotice ? <div className="rounded-[1.5rem] border border-orange-200 bg-[#fff4e8] px-5 py-4 text-sm font-black text-[#a3581c]">{planNotice}</div> : null}{renderStep()}</div>}
      />
    </div>
  );
}
