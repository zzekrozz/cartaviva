import { MapPin, MessageCircle, Phone, Instagram, Languages, Clock, QrCode } from "lucide-react";
...
export function FakeQr({ compact = false }: { compact?: boolean }) {
    ...
    <div className={`${compact ? "h-20 w-20" : "h-28 w-28"} rounded-2xl bg-white p-2 shadow-inner`}>
      <div className="grid h-full w-full grid-cols-7 gap-0.5">
        {cells.map((cell) => {
          ...
          return <span key={cell} className={`rounded-[2px] ${filled ? "bg-[#221812]" : "bg-transparent"}`} />;
        })}
      </div>
    </div>
  );
  ...
  return (
    <article className={`rounded-3xl border border-[#f0e6d8] bg-white p-3 shadow-sm ${isSoldOut ? "opacity-55 grayscale" : ""}`}>
      <div className={shouldShowImage ? "grid grid-cols-[92px_1fr] gap-3" : ""}>
        {shouldShowImage && (
          <img src={product.imageUrl || "https://images.unsplash.com/photo-1504674900247-0877df9cc836?q=80&w=900&auto=format&fit=crop"} alt="" className="h-24 w-24 rounded-2xl object-cover" />
        )}
  ...
  return (
    <div className={preview ? "bg-[#fffaf3]" : "min-h-screen bg-[#fffaf3] px-4 py-6"}>
      <div className={preview ? "overflow-hidden rounded-[2rem] bg-[#fffaf3]" : "mx-auto max-w-3xl overflow-hidden rounded-[2rem] border border-[#eadfce] bg-[#fffaf3] shadow-xl"}>
        <header className="relative h-52 bg-cover bg-center" style={{ backgroundImage: `url('${data.restaurant.coverUrl}')` }}>
        ...
        <section className="space-y-4 p-4 md:p-6">
          <div className="flex gap-2 overflow-x-auto pb-1 text-sm font-black">
            ...
            {mainCategories.map((category) => (
              <a key={category.id} href={`#${category.id}`} className="whitespace-nowrap rounded-full bg-[#f1e7d8] px-4 py-2 text-[#6b594a]">{category.name}</a>
            ))}
          </div>
            ...
            return (
              <section id={category.id} key={category.id} className="scroll-mt-4">
                <div className="mb-3 flex items-center justify-between">
                  <h2 className="text-2xl font-black text-[#221812]">{category.name}</h2>
                  <span className="text-xs font-bold text-[#8a796a]">{products.length} platos</span>
                </div>
          ...
          <div className="rounded-3xl bg-[#221812] p-5 text-white">
            <div className="flex items-center justify-between gap-4">
              ...
              <a href={whatsappHref} className="rounded-full px-4 py-3 text-sm font-black text-white" style={{ backgroundColor: data.settings.primaryColor }}>
                WhatsApp
              </a>
            </div>
          </div>
          ...
          {!preview && (
            <div className="rounded-3xl border border-dashed border-[#d9cbb8] bg-white p-5 text-center">
              <div className="mx-auto w-fit"><FakeQr compact /></div>
