...
export async function loadRestaurantState(supabase: any, restaurantId: string, ownerId: string): Promise<{ state: CartaVivaState; restaurant: RestaurantRow }> {
  ...
  const [{ data: categories, error: categoriesError }, { data: products, error: productsError }, { data: dailyMenu, error: dailyMenuError }, { data: weeklyMenus, error: weeklyMenusError }, translations, { data: landing }] = await Promise.all([
    ...
    supabase.from("restaurant_landing_settings").select("*").eq("restaurant_id", restaurantId).maybeSingle()
  ]);
  ...
  return {
    restaurant,
    state: stateFromRows(restaurant, categories as CategoryRow[], products as ProductRow[], dailyMenu as DailyMenuRow | null, (weeklyMenus || []) as WeeklyMenuRow[], translations, landing)
  };
}
...
export async function saveRestaurantState(supabase: any, state: CartaVivaState, ownerId: string, restaurantId: string, publish = false) {
  ...
  // Save landing settings (only for restaurant-pro plan; safe to upsert always)
  if (nextState.landing) {
    const landingPayload = {
      restaurant_id: restaurantId,
      enabled: nextState.landing.enabled ?? false,
      hero_title: nextState.landing.heroTitle || "",
      hero_subtitle: nextState.landing.heroSubtitle || "",
      story_title: nextState.landing.storyTitle || "",
      story_text: nextState.landing.storyText || "",
      gallery_images: nextState.landing.galleryImages || [],
      featured_product_ids: nextState.landing.featuredProductIds || [],
      reservation_url: nextState.landing.reservationUrl || "",
      google_maps_url: nextState.landing.googleMapsUrl || "",
      google_reviews_url: nextState.landing.googleReviewsUrl || "",
      seo_title: nextState.landing.seoTitle || "",
      seo_description: nextState.landing.seoDescription || "",
      landing_template: nextState.landing.landingTemplate || "warm",
      primary_cta: nextState.landing.primaryCta || "Ver carta",
      secondary_cta: nextState.landing.secondaryCta || "Cómo llegar",
    };
    // Silently ignore if table doesn't exist yet (migration not applied)
    await supabase.from("restaurant_landing_settings").upsert(landingPayload, { onConflict: "restaurant_id" });
  }
